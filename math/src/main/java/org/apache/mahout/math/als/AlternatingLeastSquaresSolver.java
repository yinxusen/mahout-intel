...
public final class AlternatingLeastSquaresSolver {
	...
	public Vector solve(Iterable<Vector> featureVectors, Vector ratingVector,
		...
		Preconditions.checkNotNull(featureVectors,
				"Feature vectors cannot be null");
		Preconditions.checkArgument(!Iterables.isEmpty(featureVectors));
		Preconditions
				.checkNotNull(ratingVector, "rating vector cannot be null");
		Preconditions.checkArgument(
				ratingVector.getNumNondefaultElements() > 0,
				"Rating vector cannot be empty");
		Preconditions
				.checkArgument(Iterables.size(featureVectors) == ratingVector
		...
		Matrix Ai = addLambdaTimesNuiTimesE(MiIi.times(MiIi.transpose()),
				lambda, nui);
	...
	/* my version of solve for MRP */
	public Vector solve(Iterable<Vector> featureVectors, Vector ratingVector,
			double lambdaPricipal, double lambdaMinor, int numFeatures) {
		...
		/* compute Ai = lambda_pricipal* MiIi * t(MiIi) + lambda_minor * nui * E */
		Matrix Ai = addLambdaTimesNuiTimesE(
				MiIi.times(MiIi.transpose()).times(lambdaPricipal),
				lambdaMinor, nui);
		/* compute Vi = MiIi * t(R(i,Ii)) */
		Matrix Vi = MiIi.times(RiIiMaybeTransposed).times(lambdaPricipal);
		/* compute Ai * ui = Vi */
		return solve(Ai, Vi);
	}
	...
	Matrix createMiIi(Iterable<Vector> featureVectors, int numFeatures) {
		Matrix MiIi = new DenseMatrix(numFeatures,
				Iterables.size(featureVectors));
		int n = 0;
		for (Vector featureVector : featureVectors) {
			for (int m = 0; m < numFeatures; m++) {
                double features = 0.0;
                try {
                    features = featureVector.getQuick(m);
                } catch(Exception e) {
                    features = 0.0;
                }
				MiIi.setQuick(m, n, features);
			}
			n++;
		}
		return MiIi;
	...
	Matrix createRiIiMaybeTransposed(Vector ratingVector) {
		...
		Matrix RiIiMaybeTransposed = new DenseMatrix(
				ratingVector.getNumNondefaultElements(), 1);
		Iterator<Vector.Element> ratingsIterator = ratingVector
				.iterateNonZero();
		int index = 0;
