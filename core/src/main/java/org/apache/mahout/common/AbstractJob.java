...
public abstract class AbstractJob extends Configured implements Tool {
	...
	protected Path getCombinedTempPath(String directory1, String directory2) {
		return new Path(new Path(tempPath, directory1) + ","
				+ new Path(tempPath, directory2));
	}
	...
	protected static Option buildOption(String name, String shortName,
		...
		return buildOption(name, shortName, description, hasArg, 1, 1,
				required, defaultValue);
	}

	protected static Option buildOption(String name, String shortName,
			String description, boolean hasArg, int min, int max,
	 ...
	 */
	public Map<String, List<String>> parseArguments(String[] args,
			boolean inputOptional, boolean outputOptional) throws IOException {
		...
		maybePut(argMap, cmdLine,
				this.options.toArray(new Option[this.options.size()]));
	...
	public int getDimensions(Path matrix) throws IOException,
		...
		try {
			reader = new SequenceFile.Reader(FileSystem.get(getConf()), matrix,
					getConf());
	...
	protected void parseDirectories(CommandLine cmdLine, boolean inputOptional,
		...
		if (outputOption != null && cmdLine.hasOption(outputOption)) {
			this.outputPath = new Path(cmdLine.getValue(outputOption)
					.toString());
			this.outputFile = new File(cmdLine.getValue(outputOption)
					.toString());
		}
	...
	protected static void maybePut(Map<String, List<String>> args,
		...
		for (Option o : opt) {
			...
			if (cmdLine.hasOption(o)
					|| cmdLine.getValue(o) != null
					|| (cmdLine.getValues(o) != null && cmdLine.getValues(o)
							.isEmpty() == false)) {
	 ...
	 * S3, hopefully this is obsolete when MultipleInputs is available again
	 */
	public static void setS3SafeCombinedInputPath(Job job, Path referencePath,
			Path inputPathOne, Path inputPathTwo) throws IOException {
	...
	protected Class<? extends Analyzer> getAnalyzerClassFromOption()
	 ...
	 * configuration resource into the provided Configuration object; note that
	 * ToolRunner calls setConf on the Tool before it invokes run.
	 */
	@Override
	public void setConf(Configuration conf) {
		...
		if (oozieActionConfXml != null) {
			...
			log.info(
					"Added Oozie action Configuration resource {0} to the Hadoop Configuration",
					oozieActionConfXml);
		}
	}
}
